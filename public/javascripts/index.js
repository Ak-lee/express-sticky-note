...
Note.prototype = {
    ...
    defaultOpts: {
        id: '',
        $container: $('#content').length > 0 ? $('#content') : $('body'),
        context: 'input here'
    },
    initOpts(opts){
    ...
    createNote(){
        ...
        this.$note.find('.username').text(this.opts.username)
         this.opts.$container.append(this.$note)
        if(!this.id){
    ...
    setLayout() {
        var self = this
        if(self.cloak){
            clearTimeout(self.cloak)
        }
        self.cloak = setTimeout(function() {
            Event.fire('waterfall')
        }, 100)
    },
    bindEvent(){
        ...
        $noteCt.on('focus', function(e) {
            if($noteCt.html() === 'input here'){
                $noteCt.html('')
            }
            $noteCt.data("before", $noteCt.html())
        }).on("blur paste", function(e) {
